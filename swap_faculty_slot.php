...
try {
    ...
    /* ================= APPLY MOVE / SWAP ================= */
    if($fromFid === $toFid) {
        throw new Exception("Cannot swap/move within the same faculty");
    }

    if($fromSlot == $toSlot && $fromDate == $toDate) {
        throw new Exception("Cannot swap/move to the same slot");
    }

    if ($fromHas && $toHas) {

        $fromSlots = $rows[$fromFid][$toDate][$toSlot] ?? [];
        $toSlots   = $rows[$toFid][$fromDate][$fromSlot] ?? [];


        if (count($fromSlots) > 0) {
            throw new Exception("Faculty $fromFid already has slots that Time");
        }

        if (count($toSlots) > 0) {
            throw new Exception("Faculty $toFid already has slots that Time");
        }
    }
    ...
    if ($fromHas && $toHas) {
        ...
        if (!isset($rows[$fromFid][$fromDate])) {
            $rows[$fromFid][$fromDate] = [];
        }

        if (!isset($rows[$toFid][$toDate])) {
            $rows[$toFid][$toDate] = [];
        }

        $rows[$fromFid][$toDate][$toSlot]       = $tempTo;
        $rows[$toFid][$fromDate][$fromSlot]     = $tempFrom;
    }

    // Slot ➜ Empty (MOVE)
    elseif ($fromHas && !$toHas) {

        throw new Exception("Scedule must be assigned to both faculties");
